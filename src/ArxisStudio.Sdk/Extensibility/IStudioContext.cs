...
/// плагины.
/// <para>
/// Свойств здесь ровно семь, и больше их не станет. Каждое новое ломает всякую
/// реализацию этого интерфейса — а реализуют его и тесты расширений, и те, кто
/// встраивает студию, — и требует старшего номера версии контракта. Растёт
/// список услуг через <see cref="GetService{T}"/>: новая служба — новая
/// регистрация, невидимая тому, кому она не нужна. Семь оставшихся — не образец,
/// а пол: то, без чего расширение не напишешь вовсе.
/// </para>
/// </remarks>
public interface IStudioContext
{
    ...
    /// <para>
    /// Своей папки у встроенного модуля нет: здесь та, где лежит его сборка, — в
    /// студии это общая для всех модулей папка <c>Modules</c>.
    /// </para>
    /// </remarks>
