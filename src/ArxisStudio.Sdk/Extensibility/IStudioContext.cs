...
public interface IStudioContext
{
    ...
    /// <summary>
    /// Путь к открытому решению или проекту; null, если проект не открыт.
    /// </summary>
    /// <remarks>
    /// Значение живое: контекст выдают один раз, а человек открывает и закрывает проекты, пока
    /// плагин живёт, — поэтому спрашивать его надо в тот момент, когда он нужен, а не запоминать
    /// при подъёме. С версии SDK 4.2 это обещание, а не случайность.
    /// <para>
    /// О самой перемене здесь не говорится: тому, кому нужно событие, нужна и модель проекта, а
    /// она приходит службой <c>IStudioProjects</c> модуля «Проекты».
    /// </para>
    /// </remarks>
    ...
    /// <para>
    /// У встроенного модуля папка тоже своя, и устроена она как у плагина: манифест в
    /// корне, сборки в <c>bin</c>. Здесь — её корень, а не место сборки.
    /// </para>
    /// </remarks>
...
public interface IStudioCommands
{
    ...
    /// <remarks>
    /// Команду, уже заявленную соседом или самой студией, занять нельзя: заявка отклоняется, а в
    /// журнал пишется имя хозяина. Свою заявить заново можно — это обновление. Идентификатору
    /// поэтому нужна приставка плагина: <c>figma.import</c>, а не <c>import</c>.
    /// <para>
    /// Обработчик зовётся в том потоке, из которого позвали команду. Меню, полоса и сочетания зовут
    /// из потока интерфейса; сосед волен позвать откуда угодно, и обработчик, трогающий контролы,
    /// обязан перейти в поток интерфейса сам.
    /// </para>
    /// </remarks>
