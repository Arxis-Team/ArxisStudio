...
public partial class MainWindow : AxWindow
{
    ...
    public MainWindow(StudioLog log)
    {
        ...
        // Оторванные окна ждут этого мига: они окна при студии, и хозяином им
        // может быть только показанное окно. До него они восстановлены, но
        // скрыты — иначе висели бы над экраном Welcome, когда рабочего места
        // ещё нет.
        //
        // Следом окну нужна каретка: без неё первая клавиша уходила в никуда. Отдаётся она после
        // раскладки — панели должны успеть встать в окно, — и туда, где её оставили в прошлый раз.
        Opened += (_, _) =>
        {
            _dock.Shown();
        ...
        Closed += async (_, _) =>
        {
            ...
            // Хост первым, документы вторыми — наоборот, чем на дорогах перезагрузки, и не по
            // недосмотру. Stop синхронный и отрабатывает наверняка: прощание панелей закрывает
            // процессы — оболочки терминала, — и не дойти до него значит оставить их сиротами.
            // А закрытие документов асинхронное: продолжение после await при закрытии окна
            // диспетчер может уже не исполнить, и всё, что стоит за ним, — под вопросом.
            _plugins.Stop();
