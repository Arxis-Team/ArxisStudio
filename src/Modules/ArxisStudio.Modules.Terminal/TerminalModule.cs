...
public sealed class TerminalModule : StudioPlugin
{
    ...
    /// <summary>
    /// Модуль выключают: оболочки закрываются вместе с ним.
    /// </summary>
    /// <remarks>
    /// За каждым сеансом стоит настоящий процесс оболочки, фоновый поток
    /// чтения и задача записи — ровно то, что <see cref="StudioPlugin"/>
    /// велит отпускать здесь. Прежде не отпускалось ничего: при закрытии
    /// студии оболочки доживали до конца процесса, а панель, снятая с
    /// раскладки, оставляла их работать невидимыми — и добраться до них было
    /// уже нечем.
    /// <para>
    /// Просит хаб, а не панель напрямую: панели у модуля нет и быть не может
    /// — её создаёт студия, — и разговор с ней идёт там же, где всегда.
    /// </para>
    /// </remarks>
