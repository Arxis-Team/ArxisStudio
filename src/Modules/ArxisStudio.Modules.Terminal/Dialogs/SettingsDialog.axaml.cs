...
/// </summary>
/// <remarks>
/// Свой диалог <b>вдобавок</b> к экрану настроек студии, а не вместо него.
/// Экран показывает те же четыре ключа — они объявлены манифестом, и этого
/// довольно, — но показывает их так, как умеет о всяком расширении: полем
/// ввода на строку и число. Оболочку же выбирают из найденных на машине, а не
/// набирают путём по памяти, и спрашивают о ней там, где на неё смотрят, — в
/// самой панели, куда экран Welcome из открытой студии не дотянуться.
/// <para>
/// Ключи и клампы общие с <see cref="TerminalSettings"/>, поэтому правка
/// отсюда и правка с экрана — одно и то же значение в одном и том же файле.
/// </para>
/// </remarks>
