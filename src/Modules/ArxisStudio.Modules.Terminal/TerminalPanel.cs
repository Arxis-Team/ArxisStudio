...
public sealed class TerminalPanel : ToolWindow
{
    ...
    /// </summary>
    /// <remarks>
    /// Зовёт студия, когда выключают модуль. За каждым сеансом — процесс
    /// оболочки, фоновый поток чтения и задача записи; за подпиской на
    /// настройки — сама панель. Не отпустив этого, студия закрывалась бы,
    /// оставляя оболочки доживать до конца процесса, а снятая с раскладки
    /// панель держала бы их работающими и невидимыми.
    /// <para>
    /// Хаб отпускается здесь же: пока в его статическом поле лежит ссылка на
    /// эту панель, живы и она, и сборка модуля за ней.
    /// </para>
    /// </remarks>
