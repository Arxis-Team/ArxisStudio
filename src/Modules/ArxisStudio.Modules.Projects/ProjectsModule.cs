...
/// <para>
/// Служб пять, и все публикуются экспортом, как у плагина: модель, сборка, пакеты, файлы и история
/// разведены по тому, чем занят берущий, — подписчику снимков события сборки не нужны, а правящему
/// ссылки не нужен снимок. Другой дороги к ним у соседей нет, и у самой студии тоже не будет —
/// работа с проектами проверяется тем путём, каким её получит чужой плагин.
/// </para>
/// </remarks>
public sealed class ProjectsModule : StudioPlugin
{
    ...
    private static void Publish(IStudioContext context, ProjectsHost host)
    {
        ...
        if (!exports.Publish<IStudioHistory>(host.HistoryService))
            context.Log.Write(StudioLogLevel.Error, LogSource, "Служба истории не опубликована: её тип уже занят");
    }
}
