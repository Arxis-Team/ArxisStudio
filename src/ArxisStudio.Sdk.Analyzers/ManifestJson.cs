...
/// зависимостей в плагин не тащит. Регулярного выражения здесь мало: поле
/// значит своё по месту. Одно и то же <c>icon</c> в голове манифеста — путь к
/// картинке карточки, а у команды — значок (<c>ARX0011</c>); секция
/// <c>toolBar</c> в комментарии или вне <c>contributions</c> — не полоса студии
/// (<c>ARX0003</c>–<c>ARX0005</c>); <c>tags</c> — только в голове (<c>ARX0007</c>),
/// а <c>%ключ%</c> в комментарии студия не прочтёт вовсе (<c>ARX0002</c>).
/// Поэтому разбор структурный, но простой —
/// скобки, строки, запятые и комментарии. Студия читает манифест с
/// комментариями и висячими запятыми, и проверка не вправе быть строже чтения —
/// и не вправе от них слепнуть.
/// <para>
