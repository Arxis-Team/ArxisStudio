...
/// вкладок и съедала бы четверть невысокого окна ради одного крестика. Крестик
/// стоит в правом краю полосы вкладок, за её пустое место окно двигают, а
/// двойным щелчком по нему разворачивают — так же это устроено и у Unity.
/// </para>
/// </remarks>
public class DockFloat : AxWindow
{
    ...
    public DockFloat()
    {
        ...
        // Visual Studio: плавающую панель там сперва пристыковывают. Кнопки
        // уборки в шапке поэтому тоже нет — её убирает сам вид.
        //
        // Кнопок окна остаётся одна — крестик. Окно при студии со всех сторон
        // палитра: своё оно не заводит ни в панели задач, ни в Alt+Tab, лежит
        // над хозяйкой и уходит вместе с ней, — а у палитры в Windows узкая
        // шапка и один крестик. Так же выглядят плавающие окна инструментов в
        // Visual Studio и Rider.
        //
        // Сворачиваться такому окну некуда: свёрнутое, оно исчезает без следа,
        // и найти его человеку нечем. Разворот же остаётся — жестом: двойной
        // щелчок по шапке разворачивает и возвращает окно, как у всякого окна
        // Windows. Жест сам себе обратный, и тупика в нём нет; пригождается он
        // на втором мониторе, где панель разворачивают во весь экран.
        View = new DockView
        {
            Actions = () => new AxWindowControls { ShowMinimize = false, ShowMaximize = false },
            Railed = false,
        };

        // Своей полосы заголовка у окна нет: полоса вкладок и есть его
        // заголовок, а крестик стоит в её правом краю. Отдельная полоса поверх
        // неё стояла бы пустой и съедала бы четверть невысокого окна ради
        // одной кнопки — так же это устроено и у Unity.
        Content = View;
