...
public sealed class DockGroup : DockNode
{
    ...
    /// <summary>
    /// Группа свёрнута: на экране от неё осталась одна полоса вкладок.
    /// </summary>
    /// <remarks>
    /// Свёрнутость — часть раскладки, а не состояние вида: она переживает
    /// перезапуск, как и место панели. Место при этом не теряется — доля
    /// группы остаётся в делении нетронутой, и разворот возвращает прежний
    /// размер, а не среднее по палате.
    /// <para>
    /// Свёрнутая группа не спрятана: полоса вкладок остаётся, и по ней панель
    /// возвращают. Спрятать её насовсем было бы нечем вернуть — полосы
    /// свёрнутых окон, как в IntelliJ, у нас нет.
    /// </para>
    /// </remarks>
