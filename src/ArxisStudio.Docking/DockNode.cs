...
public sealed class DockGroup : DockNode
{
    ...
    /// <summary>
    /// Группа убрана на рейку этой стороны; null — стоит в дереве.
    /// </summary>
    /// <remarks>
    /// Убранная группа уходит с экрана целиком, и место достаётся соседям —
    /// в этом весь смысл действия. Из дерева она при этом не уходит: доля
    /// остаётся в делении нетронутой, и возврат отдаёт прежний размер, а не
    /// среднее по палате. Показывать её нечем и не нужно — вместо неё на краю
    /// окна стоит кнопка с именем панели.
    /// <para>
    /// Сторона хранится здесь, а не выводится из формы дерева, и это не
    /// прихоть: у узлов нет ни родителя, ни поля стороны, и после перекладки
    /// «с какой она стороны» установить было бы нечем. Рейке же сторона нужна
    /// раньше, чем группа снова окажется в дереве.
    /// </para>
    /// <para>
    /// Прежде это был признак «свёрнута»: полоса вкладок оставалась на месте,
    /// а тело пряталось. Снизу выходила тонкая лента, а сбоку — пустая колонка
    /// шириной в подпись: у горизонтальной полосы вкладок нет осмысленной
    /// свёрнутой ширины, и место соседу не доставалось вовсе.
    /// </para>
    /// </remarks>
    [JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]
    public DockSide? Rail { get; init; }
