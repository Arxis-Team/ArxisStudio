...
public sealed class DockWorkspace
{
    ...
    /// <summary>
    /// Панели, закрытые человеком.
    /// </summary>
    /// <remarks>
    /// Списком, а не просто отсутствием в дереве. В дереве нет и той панели,
    /// которая ещё не вставала: плагин подняли впервые, или раскладка родом из
    /// времён, когда его не было, — такую студия ставит на объявленное место.
    /// Без этого списка закрытая возвращалась бы с каждым запуском.
    /// </remarks>
    ...
    /// <summary>
    /// Панель, в которой стояла каретка, когда раскладку записали; null — не стояла нигде.
    /// </summary>
    /// <remarks>
    /// Студия открывается туда, где её закрыли, как Rider: без этого главное окно вставало без
    /// каретки вовсе, и первая же клавиша уходила в никуда. Поле необязательное, и версия формата
    /// от него не меняется: прежняя студия незнакомое поле пропустит.
    /// </remarks>
