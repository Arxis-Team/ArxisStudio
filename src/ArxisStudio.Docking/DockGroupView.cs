...
public class DockGroupView : TemplatedControl
{
    ...
    /// теряется: <c>TemplateBinding</c> перестаёт находить свойство молча — и
    /// кнопка оставалась на виду у всех подряд, включая оторванное окно, где
    ...
    protected override void OnApplyTemplate(TemplateAppliedEventArgs e)
    {
        ...
        // тому мигу спрашивать поздно — содержимое подменят, и держать фокус
        // будет уже отцепленный контрол.
        if (_content is not null)
    ...
    private void OnChosen(object? sender, SelectionChangedEventArgs e)
    {
        ...
        // панель. Вкладка фокусируема — AxTabItem наследует ListBoxItem, — и
        // щелчок по ней уводит фокус на неё; вкладка тоже здесь, внутри
        // группы, так что ответ «да» в обоих случаях. Фокус в другой панели
        // или в другом окне даёт «нет», и красть его показ вкладки не станет.
