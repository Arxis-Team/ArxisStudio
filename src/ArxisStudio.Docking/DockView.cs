...
public class DockView : Decorator
{
    ...
    {
        var hint = Lay(ref _hint, area, static () => new Border
        {
            Classes = { "dock-hint" },
        ...
        if (hint?.Child is TextBlock label)
            label.Text = title;
    }

    /// <summary>Ставит черту, у которой встанет вкладка; null — снимает её.</summary>
    private void Mark(Rect? area) =>
        Lay(ref _caret, area, static () => new Border { Classes = { "dock-caret" }, IsHitTestVisible = false });

    /// <summary>
    /// Кладёт накладку на слой поверх окна — на место в координатах вида; null — снимает её.
    /// </summary>
    /// <param name="overlay">Накладка, если она уже заведена.</param>
    /// <param name="area">Место; null — снять.</param>
    /// <param name="make">Как завести накладку в первый раз.</param>
    /// <returns>Накладка на слое; null — снята.</returns>
    /// <remarks>
    /// Подсказка цели и черта вставки ложатся одинаково — слой, угол, размер, — и различаются только
    /// тем, что на них нарисовано.
    /// </remarks>
    private Border? Lay(ref Border? overlay, Rect? area, Func<Border> make)
    {
        if (area is not { } place
            || OverlayLayer.GetOverlayLayer(this) is not { } layer
            || this.TranslatePoint(place.Position, layer) is not { } corner)
        {
            if (overlay is { Parent: Panel host })
        ...
        if (!layer.Children.Contains(overlay))
            layer.Children.Add(overlay);

        Canvas.SetLeft(overlay, corner.X);
        Canvas.SetTop(overlay, corner.Y);
        overlay.Width = place.Width;
        overlay.Height = place.Height;
    ...
    {
        // Перестройка отцепляет все группы разом, и вместе с ними уходит тот,
        // кто держал каретку: между отпусканием и сборкой фокуса нет ни у кого.
        // Спросить надо до, вернуть — после.
        //
        // Дерево правят не только руками: показ соседней панели, возврат
        // убранной, приход плагина, восстановление раскладки — каждая такая
        // правка проходит здесь, и без этих двух строк человек терял бы фокус
        // на любой из них, даже не тронув свою панель.
        var keeper = _groups.Values.Select(group => group.FocusedPanel).FirstOrDefault(panel => panel is not null);
        ...
        if (keeper is not { } panel)
            return;

        // Presenter цепляет ребёнка на следующем проходе раскладки, а фокус
        // нельзя отдать тому, кого в дереве ещё нет: спрашивать о панели до
        // этой строки — значит всегда получать «её тут нет».
        UpdateLayout();

        // Панель могла и не вернуться на экран — её убрали, выключили плагин,
        // унесли в другое окно. Тогда возвращать некуда, и врать об этом не
        // надо: фокусом займётся тот, кто эту правку затеял.
        if (TopLevel.GetTopLevel(panel) is not null)
            DockFocus.Restore(panel);
    }
    ...
    private Control? Build(DockNode node, IReadOnlyList<int> path, DockItems items, HashSet<string> alive)
    {
        ...
        var room = DockShares.Shown(shares, [.. shown.Select(item => item.At)], division.Floor);
    ...
    {
        var visible = division.Visible;
        var was = DockShares.Shown(division.Shares, visible, division.Floor);
        var now = DockShares.Shown(next, visible, division.Floor);
    ...
    private sealed class Division(Grid grid, bool down, IReadOnlyList<int> path, int floor, IReadOnlyList<double> shares)
    {
        ...
        public void Show(IReadOnlyList<double> shares)
        {
            ...
            var room = DockShares.Shown(shares, Visible, Floor);
