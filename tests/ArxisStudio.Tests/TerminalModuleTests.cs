...
public class TerminalModuleTests
{
    ...
    /// <summary>
    /// Выключение модуля просит панель закрыть сеансы и отпускает её.
    /// </summary>
    /// <remarks>
    /// За каждым сеансом стоит процесс оболочки, фоновый поток чтения и задача
    /// записи. Прежде выключение не трогало ничего из этого: студия
    /// закрывалась, оставляя оболочки доживать до конца процесса, а панель,
    /// снятая с раскладки, держала их работающими и невидимыми.
    /// <para>
    /// Отпустить панель не менее важно, чем закрыть сеансы: держит её ссылка
    /// в статическом поле хаба, и пока она цела, живы и панель, и сборка
    /// модуля за ней.
    /// </para>
    /// </remarks>
    [Fact]
    public void Switching_the_module_off_closes_the_sessions_and_lets_the_panel_go()
    {
        TerminalHub.Reset();
        ...
        try
        {
            ...
            var host = new PluginHost(new StudioContextFactory(new StudioLog(), new StudioCommands(), null));

            Assert.True(host.LoadBuiltIn(typeof(TerminalModule).Assembly).IsLoaded);

            host.Dispose();

            Assert.Equal([TerminalRequestKind.Shutdown], received.Select(request => request.Kind));

            // Панель отпущена: следующая просьба ложится в очередь, а не идёт
            // в ту, что уже попрощалась.
    ...
    /// <summary>
    /// Прощание не копится: некому прощаться — значит и закрывать нечего.
    /// </summary>
    /// <remarks>
    /// Просьба, не застав панели, ложится в очередь и достаётся следующей.
    /// Пройди этой дорогой прощание — построенная панель первым же делом
    /// закрылась бы по просьбе прошлой жизни модуля.
    /// </remarks>
    [Fact]
    public void A_farewell_given_to_nobody_is_not_kept_for_the_next_panel()
    {
        TerminalHub.Reset();
