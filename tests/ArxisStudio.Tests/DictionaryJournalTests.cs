...
public class DictionaryJournalTests : IDisposable
{
    ...
    /// <summary>Файла перевода нет — это не порча, и чтение файла о нём молчит.</summary>
    /// <remarks>
    /// Перевода на каждый язык у плагина нет, и отсутствующий файл — обычное дело. Другое дело —
    /// словарь по умолчанию, которого у расширения нет: о нём говорит не чтение файла, не знающее,
    /// какой файл обязан быть, а словари расширения — это проверяет <c>PluginStringsTests</c>.
    /// </remarks>
    [Fact]
