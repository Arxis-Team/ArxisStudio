...
| `fg3` | `#868A91` | `#818594` | третичный текст, подписи, плейсхолдеры |
...
Теперь `fg3` — Gray8 в тёмной (4,75 / 3,98 / 3,23 на `bg1` / `bg2` / `bg3`) и
Gray7 в светлой (3,67 / 3,46 / 3,11); `fgDisabled` — Gray7 в тёмной и Gray8 в
светлой. Обе ступени с той же шкалы Int UI, ничего не выдумано.

Цена названа: **порог 4,5:1 берётся только на основном фоне тёмной темы.**
Двигать дальше некуда — следующая ступень в обоих вариантах это `fg2`, и
третичного текста не стало бы вовсе. Держится порог 3:1 на каждой поверхности,
и это закреплено тестом. Расхождение с проектом записано один раз, в
