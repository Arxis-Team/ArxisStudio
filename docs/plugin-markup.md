...
[плана](plan.md): держатель найден и снят, `{Text}` работает и без `x:Class`. Дополнен 2026-09-25,
записи 284 и 285: свои свойства и события Avalonia держат плагин до перезапуска, и студия его
предлагает — [ниже](#что-держит-плагин-до-перезапуска). Лежит отдельно от плана
...
| свой `StyledProperty`, своё событие, классовый обработчик — у плагина или у его библиотеки | — | **нет**: до перезапуска |

Последняя строка — не выбор разметки, а свойство Avalonia: заведённое типом свойство или событие она
помнит статическим реестром до конца процесса и снять не умеет. Такой плагин работает, выключается и
удаляется, но его сборки остаются в памяти до перезапуска студии. Студия это знает: выключенный,
удалённый, обновлённый или перезагруженный такой плагин ждёт перезапуска, и студия его предлагает, а
причину пишет в журнал. [Подробности ниже](#что-держит-плагин-до-перезапуска).
...
забывает по списку типов их переопределения метаданных и кэши. Записей, заведённых самим типом, — его
свойств, — он не снимает: замер 2026-09-15 этого не видел, его контролы своих свойств не заводили
([ниже](#что-держит-плагин-до-перезапуска)). С ним выгружаются все дороги того замера, два прогона из
двух, без единого обращения к внутренностям:
...
## Чужая библиотека контролов везёт своё оформление

Плагин вправе взять чужую библиотеку контролов — она едет в его пакете и грузится его же
контекстом. Вместе с ней приезжает её оформление, и написано оно почти наверняка под одну
из тем Avalonia: под контролами студии стоит своя, и ни Fluent, ни Simple в ней нет.
Найдено это на просмотрщике [src/Plugins/Arxis.CodeViewer](../src/Plugins/Arxis.CodeViewer),
первом таком плагине, — запись 276.

**Оформление объявляют у своего контрола, а не у приложения.** Стиль, добавленный в
`Application.Styles`, оценивался бы на каждом контроле студии и пережил бы выгрузку
плагина; `ARX0014` говорит об этом при сборке. У вкладки он живёт ровно столько, сколько
она:

```csharp
var theme = new Uri("avares://AvaloniaEdit/Themes/Fluent/AvaloniaEdit.xaml");
var style = new StyleInclude(theme) { Source = theme };
```

**Недостающие ключи кладут в словарь самого оформления.** Первый же замер такого контрола
роняет `KeyNotFoundException: Static resource 'ControlContentThemeFontSize' not found` — и
вкладка не открывается вовсе: шов студии считает это сбоем открытия и говорит человеку,
что редактор не справился. Положить ключ рядом с контролом мало: `StaticResource` в
разметке ищет по тому, что стояло вокруг него в документе, а не по дереву контролов, и до
ресурсов вкладки не доходит. Дорога — словарь загруженного стиля, и раньше, чем контрол
спросит своё оформление:
...
`Bridge` здесь — пары «чужое имя — ключ темы» (`ControlContentThemeFontSize` →
`AxFontSize`). Правило значений это не нарушает: слева чужое имя, справа ключ, своего
числа не появляется.

**Голый контрол Avalonia в студии не одет ничем.** Тема одевает свои `Ax*`, окно, окно
попапа, простые контейнеры и полосу прокрутки; `TextBox` и `Button`, взятые прямо у
Avalonia, остаются без шаблона — их не видно. Чужая библиотека сплошь и рядом строит из
них своё: у AvaloniaEdit так сделана панель поиска, и в студии она вышла тремя значками
без фона и без поля ввода. Выбор за автором плагина: одеть их своим `ControlTheme` на
ключах темы — или снять то, что они показывают. Просмотрщик снимает панель поиска: он
показывает файл, а не правит его.

**Данные чужой библиотеки красят так же — ключами.** Подсветка синтаксиса цветом в
разметке не называется и под `ARX0008` не попадает: цвета лежат в наборе, который пакет
привёз с собой, и подобраны под его тему. Правило от этого не меняется — плагин
переписывает набор ролями темы, а не оставляет чужое. У просмотрщика это
[`CodePalette`](../src/Plugins/Arxis.CodeViewer/CodePalette.cs): роль ищется по имени
цвета, которое дал автор подсветки, незнакомое имя остаётся непокрашенным и берёт цвет
самого редактора, а обход идёт по всему графу набора — именованных цветов мало, и часть
объявлена прямо в правилах. Снимается и то, что цветом не является: чужой кегль и чужая
гарнитура. Значения берутся наблюдателем ресурсов, поэтому подсветка идёт за темой, а
подписку вкладка отпускает, прощаясь.

## Что держит плагин до перезапуска

Замер 2026-09-25, запись 284. Каждый плагин репозитория поставлен из своего архива, задействовано всё,
что он даёт, — документ, каретка в панели, команды, пункт «Добавить», экспорт соседа, — потом
выключение или перезагрузка. Контекст считается утёкшим, только если жив и после прокрутки отрисовки,
а держатель ищется обходом графа от статических полей по объектам одного этого контекста:

- Hello и Hello Friend выгружаются — после записи 283; у Deutsch сборки нет;
- **Code Viewer не выгружается** — ни выключением, ни перезагрузкой, ни после закрытия документа.
  Держат его регистрации AvaloniaEdit в статических реестрах Avalonia:
  - `AvaloniaPropertyRegistry` — 38 свойств, 7 типов-владельцев (`TextView`, `TextEditor`,
    `TextArea` …), `_direct[TextArea]` и присоединённое `LayerPosition` под типом-хозяином `Control`;
  - `RoutedEventRegistry` — `TextView` и `RoutedCommand`;
  - классовые обработчики на объектах самой Avalonia: `InputElement.GotFocusEvent`,
    `TextInputMethodClientRequestedEvent`, `TextElement.FontFamily/FontSize.Changed`. Первый вешает
    `RoutedCommand` — на все `Interactive`, и пишет последний сфокусированный элемент в своё
    статическое поле: код «выгруженного» плагина продолжает работать на каждой смене фокуса и держит
    чужой контрол — в пробе это была кнопка из панели Hello.

  Снятые рефлексией, эти записи отпускают контекст: список полный.

AvaloniaEdit тут не особенный. Два собранных на ходу плагина без чужих библиотек остаются так же:
контрол с одним своим `StyledProperty`, которым он пользуется, и плагин с `AddClassHandler` на
`GotFocusEvent`. Причина в Avalonia, одна и та же в 12.1.1 и в основной ветке:

- `UnregisterByModule` в `_registered`, `_attached` и `_direct` обходит свойства и чистит у них
  метаданные типа, а ключ самого типа и его свойства в `_properties` оставляет — и отвечает `true`;
- у `RoutedEventRegistry` снятия нет вовсе, только `Register` и чтение;
- классовый обработчик — подписка на субъект внутри события или свойства, а `IDisposable`, который
  вернул `AddClassHandler`, библиотеки не хранят.

Замер 2026-09-15 этого не видел: его контролы своих свойств не заводили. Там же ловушка для проб: у
типа без явного статического конструктора (`beforefieldinit`) инициализатор поля-свойства зовётся при
первом обращении к полю, и контрол, который своё свойство ни разу не тронул, его и не заводит.

**Что делает студия.** Дочищать реестры Avalonia рефлексией на запись она не берётся — по решению
владельца. Она перезапускается, и так, как Rider:

- спуск, который не довёл дело до конца, ставит плагин в ждущие перезапуска
  (`StudioPlugins.AwaitingRestart`). Поводов два: прежняя копия осталась в памяти и контракт
  пересобран, а общий контекст его не обновит. Кого держат реестры, видно сразу — `PluginHost.Pinned`
  называет сборку, заведшую свои свойства и события, и её типы. Застрявшему без видимой причины
  даётся передышка: до трёх кругов из паузы, прохода диспетчера и сборки мусора;
- человеку — ни типов, ни сборок. В менеджере у такого плагина «Требуется перезапуск ArxisStudio» и
  ссылка «Перезапустить», в списке — значок, в строке состояния студии — то же с той же ссылкой. После
  «Сохранить», удаления, установки и перезагрузки студия спрашивает: «Перезапустить ArxisStudio,
  чтобы применить изменения в плагинах?» — [Перезапустить] и [Не сейчас]. Спрашивает только о новом
  поводе: «Не сейчас» закрывает вопрос, а не нужду;
- причина — в журнале, `Warning` в разделе `Plugins`: сборка первой, у просмотрщика держит не его код,
  а AvaloniaEdit. Перезапущенная копия пишет её в свой журнал ещё раз, разделом `Restart`: прежний
  умер вместе с процессом;
- перезапуск возвращает открытое: проект, документы и выбранные вкладки, каретку, место главного окна
  и окно настроек с его разделом, поиском и выбранным плагином. Дорога процессов и что не
  возвращается — в [devtools.md](devtools.md#перезапуск).

Свойства спрашиваются закрытым словарём реестра `_registered`, **только на чтение**: под
типом-владельцем Avalonia кладёт и обычное свойство, и прямое, и присоединённое, и принятое `AddOwner`.
Открытый `GetRegistered` прогоняет статические конструкторы всей цепочки типов и сам завёл бы свойства,
которых плагин ещё не трогал: вопрос приковал бы к памяти того, о ком спрашивает. Это держит тест —
копию, о которой спросили до того, как её код что-то завёл, он выгружает. Пропадёт словарь при
обновлении Avalonia — ответ обеднеет, а не соврёт, и упадёт тест. События спрашиваются открытым
`GetAllRegistered`. Классовые обработчики так не видны вовсе; их ловит проверка после спуска с
передышкой, и плагин встаёт в ждущие перезапуска с причиной «на её типы ещё кто-то ссылается».

**Автору плагина:**

- свой `StyledProperty`, `DirectProperty`, присоединённое свойство, `AddOwner` или своё `RoutedEvent`
  — плагин рабочий, но обновляется он перезапуском. Состоянию контрола, которому не нужны стили и
  привязки, хватает обычного свойства C#;
- классовый обработчик на общем событии или свойстве снимают сами: `IDisposable` от `AddClassHandler`
  хранят и отпускают в `Deactivate`;
- чужая библиотека контролов обычно делает всё это сама, как AvaloniaEdit, и тогда выбора у автора
  нет.
...
Открытых два, и оба — кандидаты в замечание самой Avalonia:

- кэш метаданных с сильными ключами-типами. Студию он не держит: пока `UnregisterByModule` на месте,
  этого хватает. Пропади он при обновлении Avalonia — не соберётся студия; перестань он снимать кэш —
  упадёт тест выгрузки;
- свои свойства, события и классовые обработчики типа, которых Avalonia не снимает вовсе. Тут студия
  не лечит, а перезапускается — записи 284 и 285, [выше](#что-держит-плагин-до-перезапуска). Появись у
  Avalonia снятие — плагины со своими контролами станут перезагружаться на ходу без единой правки у их
  авторов.
