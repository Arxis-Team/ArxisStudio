...
по определению. `Modules.Terminal`, `Modules.Console` и `Modules.Sample` написаны разметкой и
правы.
...
- **Хозяин `avares://` — простое имя сборки, одно на процесс.** Два плагина со сборками
  ...
  словами.
- `Assets/` (под avares) и `assets/` (каталог пакета) на Windows совпадают, и файл уезжает
