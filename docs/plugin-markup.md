...
Правые две клетки таблицы стали правдой не сразу: до записи 115 перезагрузку терял и
тот, кто своего типа-контрола не заводил, — хватало одного обращения к `avares://`.
Причина была вторая и наша, [про неё ниже](#отдельно-кэш-загрузчика-ресурсов--поставлено).
...
**Плагину с `x:Class` это не помогло и помочь не могло** — его держит тип-контрол, и
замер это разделяет: утечка типа остаётся и после чистки кэша. Вывод всей записки —
разметка с `x:Class` стоит перезагрузки на ходу — в силе; снято одно лишнее слагаемое,
из-за которого перезагрузку терял и тот, кто своего типа-контрола не заводил.
...
Закрыто: **сброс кэша загрузчика ресурсов** — поставлен записью 115 и держится тестом.
