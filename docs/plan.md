...
1. **M0 — Controls + Themes** ✅ *(2026-08-24)*: репозитории ArxisStudio.Controls и
   ...
   каждом этапе.
2. **M1 — каркас**: субмодули, solution, Shell (окно, тема, зоны), приложение запускается.
3. **M2 — Welcome**: recents (персистентно), открытие .sln/.csproj, создание из
   dotnet new шаблонов, Settings, Plugins-менеджер (локальный), Learn-статика.
