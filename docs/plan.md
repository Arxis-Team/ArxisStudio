...
ArxisStudio/                        ← главный репо (Avalonia-приложение + новые библиотеки)
  ...
  src/
    ...
    Modules/                        ← встроенные модули студии
      ArxisStudio.Modules.Designer/ ← канва, иерархия, палитра, инспектор, вкладка XAML
      ArxisStudio.Modules.Project/  ← дерево решения
      ArxisStudio.Modules.Console/  ← журнал студии и проблемы
    ArxisStudio/                    ← оболочка: Welcome, зоны, вкладки, запуск
  tests/
```
...
- **Editor**: зоны, вкладки документов, заголовок, статус-бар — и ни одной панели.
  Иерархия, палитра, канва, инспектор, вкладка XAML, дерево проекта, консоль и
  проблемы приходят встроенными модулями через тот же контракт SDK, что и внешние
  плагины: студия собрана из модулей и проверяет контракт на себе.
- Build/Run: сборка через ProjectSystem, запуск процесса с потоком stdout в журнал;
  вкладку вывода оболочка находит по роли `output` из манифеста.
