...
папок проектов должно быть только назначение. И занятое назначение можно заменить, а не получить
...
именем и при отказе возвращается на место. Занятая папка остаётся отказом — сливать папки служба не
берётся.
...
историю пути от новой строки к старой: у файла — сквозь его переименования и переезды папки, в
которой он лежал, у папки — всё, что в ней появлялось, пропадало, менялось, приезжало и уезжало, и
