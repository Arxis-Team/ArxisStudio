...
- **Плагины** — панель, команда и служба; свои рисовальщики свойств и свои
  инспекторы для типов контролов; каждый плагин в своём выгружаемом контексте
  загрузки, и сбой одного не роняет студию.
- **Сборка и запуск** — вывод `dotnet` и самого приложения построчно в «Консоль».

Чего пока нет: меню из манифеста, активация плагина по событию (включённый
поднимается сразу) и экран Run/Debug с горячей перезагрузкой.
