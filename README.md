...
(`src/ArxisStudio.Extensibility`) и пример плагина (`samples/Arxis.HelloPlugin`).
...
Способ доставки «встроенный модуль» — панель, приезжающая вместе со студией, — хост
по-прежнему знает, но ни одного модуля в репозитории сейчас нет: контракт
проверяется тем, ради чего он сделан, — внешним плагином.
